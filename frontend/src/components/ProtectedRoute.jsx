...
function ProtectedRoute({ allowedRoles }) {
  ...
  if (isLoading) {
    // Можно показать полноэкранный лоадер
    return <div>Загрузка приложения...</div>; // TODO: Заменить на красивый лоадер
  }

  // Если пользователь не аутентифицирован после завершения загрузки
  if (!isAuthenticated) {
  ...
  if (allowedRoles && allowedRoles.length > 0) {
    // Проверяем, существует ли объект пользователя и входит ли его роль в список разрешенных allowedRoles
    // Если user null или его роль не найдена в массиве allowedRoles
