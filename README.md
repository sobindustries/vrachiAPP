...
*   Регистрация пользователя с подтверждением email (ссылка для подтверждения выводится в консоль бэкенда).
...
*   **Фронтенд (Web):** React, Vite, SASS (для общих стилей), Material UI (готовые UI компоненты), React Router (роутинг), Axios (HTTP-клиент), Zustand (управление состоянием).
...
3.  **Настройте переменные окружения:**
    ...
    *   Откройте файл `.env` и добавьте следующие переменные. **Обязательно замените значения на свои надежные данные!**
        ...
        # Базовый URL для подтверждения email на фронтенде (используется бэкендом в заглушке письма)
...
## Обновления в текущей версии (1.0.1)
...
*   Реальная отправка писем для подтверждения email.
